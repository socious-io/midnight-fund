...
import { CrowdfundingAPI, type DeployedCrowdfundingAPI } from "@crowdfunding/crowdfunding-api";
...
const DeployedContractProvider = ({ 
  ...
  const onDeployContract = useCallback(async () => {
    ...
    try {
      const deployedAPI = await CrowdfundingAPI.deployCrowdfundingContract(
        walletContext.walletState.providers,
        logger
      );
